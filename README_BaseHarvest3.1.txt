...
Extension-Base-Harvest divides the landscape into user-defined management areas for timber
harvesting. That is, management areas define collections of stands to which specific harvesting 
prescriptions can be applied. An unlimited number of management areas can be defined. Management 
areas need not be contiguous and management areas need not have any harvesting prescriptions 
implemented, thereby remaining essentially "non-active" from the harvest perspective.

Extension-Base-Harvest requires that harvest prescriptions be defined that specify several harvesting 
criteria and which species cohorts will be targeted. Prescriptions determine which stands within a
management area qualify for harvest, and define the order (ranking) in which these stands will be 
harvested. Separate prescription rankings are derived for each management area. For each prescription
in a management area, Extension-Base-Harvest begins harvesting in the highest ranked stand and 
proceeds down the ranked list of stands until the desired percentage of area in the management area
has been cut. Prescriptions can be shared across management areas.
